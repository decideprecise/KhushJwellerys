<?php
/**
 * Rara eCommerce Customizer Notification Section Class.
 *
 * @package Rara_eCommerce
 */

/**
 * Rara_eCommerce_Customizer_Notice_Section class
 */
class Rara_eCommerce_Customizer_Notice_Section extends WP_Customize_Section {
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'rara_ecommerce_cust_notice_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'rara_ecommerce_cust_notice_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		$json = parent::json();
		global $rara_ecommerce_recommended_plugins;
		...
		$show_recommended_plugins = get_option( 'rara_ecommerce_show_recommended_plugins' );

		foreach ( $rara_ecommerce_recommended_plugins as $slug => $plugin_opt ) {
			...
			$rara_ecommerce_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$rara_ecommerce_recommended_plugin['class'] = 'active';
			} else {
				$rara_ecommerce_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$rara_ecommerce_recommended_plugin['button_class'] = 'install-now button';
					$rara_ecommerce_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$rara_ecommerce_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$rara_ecommerce_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$rara_ecommerce_recommended_plugin['button_class'] = 'deactivate-now button';
					$rara_ecommerce_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
            
			$info   = $this->call_plugin_api( $slug );
			$rara_ecommerce_recommended_plugin['id'] = $slug;
			$rara_ecommerce_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$rara_ecommerce_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$rara_ecommerce_recommended_plugin['description'] = $info->short_description;
			}

			$rara_ecommerce_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $rara_ecommerce_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="rara-ecommerce-recommeded-actions-container rara-ecommerce-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="rara-ecommerce-recommeded-actions">
