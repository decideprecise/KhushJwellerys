<?php
/**
 * Rara eCommerce Customizer Notification System.
 *
 ...
 */
class Rara_eCommerce_Customizer_Notice {
	...
	/**
	 * The single instance of Rara_eCommerce_Customizer_Notice
	 *
	 * @var Rara_eCommerce_Customizer_Notice $instance The Rara_eCommerce_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main Rara_eCommerce_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of Rara_eCommerce_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Rara_eCommerce_Customizer_Notice ) ) {
			self::$instance = new Rara_eCommerce_Customizer_Notice;
	...
	public function setup_config() {
		...
		$rara_ecommerce_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$rara_ecommerce_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_enqueue_style( 'rara-ecommerce-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/css/customizer-notice.css', null );
		...
		wp_enqueue_script( 'rara-ecommerce-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/js/customizer-notice.js', array( 'jquery', 'customize-controls' ), RARA_ECOMMERCE_THEME_VERSION, true );
		wp_localize_script(
			'rara-ecommerce-customizer-notice', 
            'customizer_notice_data', 
            array(
				'ajaxurl'            => admin_url( 'admin-ajax.php' ),
				'template_directory' => get_template_directory_uri(),
				'base_path'          => admin_url(),
				'activating_string'  => __( 'Activating', 'rara-ecommerce' ),
			)
		);
	}
	...
	public function customize_register_notice( $wp_customize ) {
		/**
		 * Include the Rara_eCommerce_Customizer_Notice_Section class.
		 */
		...
		$wp_customize->register_section_type( 'Rara_eCommerce_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new Rara_eCommerce_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			update_option( 'rara_ecommerce_show_recommended_plugins', $show_recommended_plugins );
		}
